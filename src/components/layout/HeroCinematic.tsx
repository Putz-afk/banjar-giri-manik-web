...
export function HeroCinematic() {
  return (
    <div className="relative w-full h-screen flex flex-col justify-center items-center overflow-hidden">
      ...
      {/* 1. BACKGROUND LAYER (The Showcase) */}
      {/* Replace this div with <Image /> or <video /> later */}
      ...
      {/* 2. CENTER CONTENT (For Outsiders) */}
      <div className="relative z-20 text-center space-y-6 max-w-3xl px-4 animate-in fade-in slide-in-from-bottom-8 duration-1000">
        ...
        <h1 className="text-5xl md:text-7xl font-bold text-white tracking-tight leading-tight drop-shadow-lg">
          Pura Giri Kusuma <br/>
          {/* 'Sorowako' is with different styling */}
        ...
        <div className="flex gap-4 justify-center pt-4">
          <Button className="bg-white text-slate-900 hover:bg-slate-200 rounded-full px-8 h-12 text-base">
            Tentang Kami
          </Button>
          <Button variant="outline" className="bg-white text-slate-900 hover:bg-slate-200 rounded-full px-8 h-12 text-base">
            Galeri Foto
          </Button>
        </div>
      </div>

      {/* 3. FLOATING EVENT WIDGET (For Insiders) */}
      {/* Positioned absolute at bottom right */}
      <div className="absolute bottom-8 right-4 md:right-12 z-30 w-full max-w-xs">
